...
async function renderHiggsfieldVideo(ctx: MediaContext, credentials: ProviderConfig, onProgress?: ProgressFn): Promise<RenderResult> {
  ...
  // Endpoint keys off the catalogue id (ctx.model), not the alias-resolved
  // wireModel — aliasing only rewrites the request-body model name.
  ...
  return {
    bytes,
    providerNote: `higgsfield/${ctx.wireModel} · i2v · ${durationSec}s · ${bytes.length} bytes`,
    suggestedExt: '.mp4',
  };
}
