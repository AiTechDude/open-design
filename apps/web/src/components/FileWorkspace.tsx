...
type DesignSystemReviewDetails = Pick<
  NonNullable<ProjectMetadata['designSystemReview']>[string],
  'feedback' | 'files' | 'agentTask'
>;
...
export function FileWorkspace({
  ...
  function closeTab(name: string) {
    ...
    setActiveTab(nextActive ?? defaultRootTab);
    setSketches((curr) => {
      const next = { ...curr };
...
function designSystemSectionChangedAfterReview(
  ...
  const trackedNames = reviewEntry.files && reviewEntry.files.length > 0
    ? reviewEntry.files
    : names;
...
function designSystemProjectFileIcon(name: string): 'file-code' | 'image' | 'file' | 'palette' {
  if (name === 'DESIGN.md' || name === 'colors_and_type.css') return 'palette';
  if (/\.(html?|css|tsx?|jsx?|json)$/i.test(name)) return 'file-code';
  if (/\.(svg|png|jpe?g|gif|webp)$/i.test(name)) return 'image';
  return 'file';
}
